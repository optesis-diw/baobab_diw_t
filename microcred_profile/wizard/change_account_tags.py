...
class WizardChangeAccountTagsLine(models.TransientModel):
    ...
    axis_id = fields.Many2one(comodel_name='account.axis', string='Axis', readonly=True, help='Select the axis.')
    application = fields.Selection(selection=[('optional', 'Optional'), ('required', 'Required')], readonly=True)
    tag_ids = fields.Many2many(comodel_name='account.axis.tag', string='count Tags', help='Select the tag(s).')
    number = fields.Integer(string='Axis number', help='Select the axis number.', readonly=True)
