...
class WizardExportSage(models.TransientModel):
    ...
    date_from = fields.Date(string='sage From date ', help='Select the date from which the export should take data into account.')
    date_to = fields.Date(string='sage date to', help='Select the date from which the export should take data into account.')
    period_from_id = fields.Many2one(comodel_name='account.period', string='sage periode From', help='Select the period from which the move lines should be taken into account.')
