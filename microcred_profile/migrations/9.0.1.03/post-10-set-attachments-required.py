...
__name__ = 'Set attachments required'
...
def migrate(cr, v):
    ...
    invoice_obj = pool.get('account.move')
    move_ids = invoice_obj.search(cr, uid, [('type', 'in', ('out_invoice', 'out_refund'))])
    print "Invoices :", len(move_ids)
    invoice_obj.write(cr, uid, move_ids, {'attachment_required': False})
