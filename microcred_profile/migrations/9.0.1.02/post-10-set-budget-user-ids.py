...
def migrate(cr, v):
    ...
    element_ids = element_obj.search(cr, uid, [('type', 'in', ('periodic', 'partner', 'department', 'project'))])
    print "Budget elements :", len(element_ids)
    for element in element_obj.browse(cr, uid, element_ids):
        print element.id
        element_obj.write(cr, uid, element.id, {'user_id': element.user_id.id})
