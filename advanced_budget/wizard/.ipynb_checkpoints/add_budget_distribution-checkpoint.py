...
class WizardAddBudgetDistributionLine(models.TransientModel):
    ...
    amount_fixed = fields.Float(string='Fixed amount', digits_compute="Account", help='Enter the fixed amount.')
    percentage = fields.Float(string=' budget dist Percentage', digits_compute="Account", help='Enter the percentage (if there is no fixed amount).')
    amount_calculated = fields.Float(string='Calculated amount', digits_compute="Account", help='Enter the calculated amount.', compute='_get_calculated_amount', )
    amount_parent = fields.Float(string='Parent\'s amount', digits_compute="Account", related='wizard_id.amount')
...
class WizardAddBudgetDistribution(models.TransientModel):
    ...
    amount = fields.Float(string='Amount', digits_compute="Account", help='The planned amount.', readonly=True, )
